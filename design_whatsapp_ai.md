...
- **Keamanan:** Admin panel di Next.js dilindungi autentikasi ketat agar hanya pengguna yang bisa mengakses Knowledge Base. Data di Supabase diproteksi menggunakan RLS.
...
- **Arsitektur Sistem:** Beralih ke **Baileys Native**. Menjalankan script Node.js secara persisten (`npm run bot`) bersamaan dengan aplikasi Next.js (Admin UI). Keduanya berbagi database Supabase yang sama.
...
1. **Baileys Bot (Backend Khusus):** Script Node.js yang bertugas menjaga WebSocket WhatsApp, menghasilkan QR Code (di terminal), membaca pesan masuk (`messages.upsert`), menjalankan timer 10s, mencari fakta ke Supabase, dan berinteraksi dengan API Gemini.
...
4. **Google Gemini API:** LLM untuk menghasilkan teks (*generation*) dan representasi semantik (*embedding*).
...
- Next.js memanggil model Embedding Gemini untuk mengubah teks jadi vektor.
- Vektor disimpan di tabel Supabase (`pgvector`).
- Saat ada pertanyaan masuk di WhatsApp, bot Baileys memanggil Supabase untuk melakukan pencarian *Cosine Similarity*. Fakta relevan digabungkan bersama *System Prompt* ke Gemini AI untuk di-*generate* jawabannya.

### D. Fitur Admin & Error Handling
- **Admin UI:** Terdapat Knowledge Base Manager (CRUD), Persona Editor, dan log obrolan AI *read-only*.
- **Error Handling:** Baileys secara otomatis me-reconnect jika putus. Jika API Gemini atau Supabase gagal/timeout, Bot akan membatalkan pembalasan (AI diam) agar terhindar dari pengiriman pesan *error message* yang kaku ke lawan bicara.
