    ...
    <form onSubmit={handleSave} className="rounded-[2rem] border border-white/8 bg-[#0e1c2d] p-5 shadow-xl shadow-black/10 sm:p-7">
      <div>
        ...
        <textarea 
          ...
          rows={10}
          className="mt-6 min-h-64 w-full resize-y rounded-2xl border border-white/10 bg-slate-950/45 px-4 py-3 font-mono text-sm leading-6 text-slate-200 placeholder:text-slate-600 outline-none transition focus:border-teal-300/60"
        />
      </div>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center"><button 
        type="submit" 
        disabled={loading}
        className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-teal-300 px-5 text-sm font-bold text-slate-950 transition hover:bg-teal-200 disabled:cursor-not-allowed disabled:opacity-50"
      >
        {loading ? <LoaderCircle className="h-4 w-4 animate-spin" aria-hidden="true" /> : <Save className="h-4 w-4" aria-hidden="true" />}
        {loading ? 'Menyimpan...' : 'Simpan perubahan'}
      </button>{saved && <span className="inline-flex items-center gap-2 text-sm font-medium text-teal-200"><Check className="h-4 w-4" aria-hidden="true" /> Tersimpan</span>}</div>
    </form>
  );
}
