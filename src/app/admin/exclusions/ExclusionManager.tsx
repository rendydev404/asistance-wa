...
export default function ExclusionManager({ initialItems }: { initialItems: ExcludedContact[] }) {
  ...
  return (
    <div className="space-y-5">
      <form onSubmit={addNumber} className="rounded-[2rem] border border-violet-200/10 bg-[linear-gradient(135deg,rgba(139,92,246,0.14),rgba(14,28,45,0.96)_48%)] p-5 shadow-xl shadow-black/10 sm:p-6">
        <div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-violet-300/12 text-violet-200"><Ban className="h-5 w-5" aria-hidden="true" /></span><div><h2 className="text-lg font-semibold text-white">Tambah nomor</h2><p className="mt-1 text-sm leading-5 text-slate-400">AI akan diam untuk nomor ini sampai pengecualiannya dimatikan.</p></div></div>
        <div className="mt-6"><label className="mb-2 block text-sm font-semibold text-slate-200" htmlFor="excluded-phone">Nomor WhatsApp</label><input id="excluded-phone" required type="tel" value={phoneNumber} onChange={(event) => setPhoneNumber(event.target.value)} placeholder="0812 3456 7890" className="min-h-12 w-full rounded-2xl border border-white/10 bg-slate-950/45 px-4 text-sm text-white placeholder:text-slate-600 outline-none transition focus:border-violet-200/60" /><p className="mt-2 text-xs text-slate-500">Bisa ditulis 08..., 62..., atau +62...</p></div>
        ...
        <button type="submit" disabled={loading} className="mt-5 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-violet-300 px-5 text-sm font-bold text-slate-950 transition hover:bg-violet-200 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto">
      ...
      <div className="overflow-hidden rounded-[2rem] border border-white/8 bg-[#0e1c2d]">
        <div className="flex items-center justify-between border-b border-white/8 px-5 py-4"><div><h2 className="font-semibold text-white">Daftar pengecualian</h2><p className="mt-1 text-xs text-slate-500">{items.filter((item) => item.active).length} nomor sedang dilewati</p></div><Check className="h-5 w-5 text-violet-300" aria-hidden="true" /></div>
          ...
          {items.length === 0 ? <p className="p-6 text-sm text-slate-500">Belum ada nomor. Tambahkan nomor admin atau pelanggan yang perlu ditangani manual.</p> : items.map((item) => (
            <div key={item.id} className="flex items-center justify-between gap-3 p-5">
              ...
              <div className="flex items-center gap-3">
                <button type="button" onClick={() => toggleNumber(item)} className={`min-h-11 rounded-xl px-3 text-xs font-bold transition ${item.active ? 'bg-violet-300/12 text-violet-200 hover:bg-violet-300/20' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}>{item.active ? 'Aktif' : 'Nonaktif'}</button>
